...
import { decodeEventLog, encodeAbiParameters, parseAbiItem, zeroAddress } from "viem";
...
import { genProofs, getPoll, mergeMessages, mergeSignups, proveOnChain, publish, signup, timeTravel, verify } from "maci-cli";
...
describe("Sniper", function () {
  ...
  describe("Complete Sniper zone", function () {
    ...
    it("Should complete zone and trigger hook mint reward correctly", async function () {
      ...
      await sniperAsOther.write.createSniperZone([15n * 60n, BigInt(await time.latest()), "bafkreicj2q3emv3qzn73r6junvfysb7ua5hyt2jctx3rumx3fhnj3nki3u"]);
      await time.increase(60 * 60);
      expect(await rewardToken.read.balanceOf([otherAccount.account.address])).to.eq(0n);
      ...
      expect(await rewardToken.read.balanceOf([otherAccount.account.address])).to.eq(8333n * 10n ** 15n);
    });
  ...
  describe("Party", function () {
    ...
    it("Should claim funds", async function () {
      ...
      const beforeSignup = BigInt((await signer.provider?.getBlock('latest'))?.number as number)
      ...
      const logs = await publicClient.getLogs({
        ...
        args: {
          _userPubKeyX: BigInt(pubKey.asContractParam().x),
          _userPubKeyY: BigInt(pubKey.asContractParam().y),
        },
        fromBlock: beforeSignup,
        toBlock: 'latest'
      })
      console.log(logs)
