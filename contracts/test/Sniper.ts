  ...
  describe("Create Sniper zone", function () {
    ...
    describe("Party", function () {
      ...
      it("Should sponsor a party and get party token", async function () {
        ...
        const party = await sniperPartyManager.read.activeParties([0n]);
        const sniperPartyCredit = await hre.viem.getContractAt(
          "SniperPartyCredit",
          party[2]
        );
        ...
        const sniperPartyCreditBalance = await sniperPartyCredit.read.balanceOf([owner.account.address]);
        expect(sniperPartyCreditBalance).to.equal(usdcAmount);
      });
